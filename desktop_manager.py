...
# -*- coding: utf-8 -*-
"""
Smart Desktop v1.4 — Window Manager with Spaces
✅ КРИТИЧЕСКИЕ ИСПРАВЛЕНИЯ:
1. Кнопки пространств: пересоздание вместо configure() для customtkinter
2. Режим "Все окна": не ломает saved_rect, окна восстанавливаются корректно
3. Ховер-эффекты: безопасные цвета для transparent
"""
...
WATER_COLORS = {
    ...
    "ripple": "#00d4ff",
    "main_ripple": "#ff8c42",
    "transparent": "transparent",  # ✅ Добавили для безопасности
}
...
class SmartDesktop(ctk.CTk):
    def __init__(self):
        ...
        self._all_windows_was_active = False
        self._pre_all_windows_state = None  # ✅ СОХРАНЯЕМ СОСТОЯНИЕ ПЕРЕД "ВСЕ ОКНА"
        ...
        self.own_hwnd = None
        try:
            def enum_find(hwnd, _):
                try:
                    if win32gui.GetWindowText(hwnd) == "Smart Desktop":
                        self.own_hwnd = hwnd
                        return False
                except:
                    pass
                return True
            win32gui.EnumWindows(enum_find, None)
        except Exception as e:
            logger.warning(f"Could not find own hwnd: {e}")
        self._load_config()
        ...
        logger.info("=== Smart Desktop v1.4 Started ===")
    ...
    def _refresh_theme_ui(self):
        ...
        if hasattr(self, 'close_btn'):
            self.close_btn.configure(fg_color="#4709f0", hover_color="#c0392b")
        ...
        # ✅ ПЕРЕ СОЗДАНИЕ КНОПОК ПРОСТРАНСТВ для обновления цветов
    ...
    def _init_flag_button(self):
        ...
        self._add_hover_effect(self.flag_btn, "accent", "accent_glow")
    ...
    def _init_header_normal(self):
        ...
        self._add_hover_effect(self.settings_btn, "glass", "glass_border")
    ...
    def _init_right_buttons(self):
        ...
        self._add_hover_effect(self.all_windows_btn, 
            "accent" if self.show_all_windows_mode else "glass",
            "glass_border")
        ...
        self._add_hover_effect(self.save_btn, "glass", "glass_border")
        self.apply_btn = ctk.CTkButton(
            ...
            command=lambda: self._apply_space_layout(self.cur_space)
        )
        self.apply_btn.pack(side="right", padx=5)
        self._add_hover_effect(self.apply_btn, "accent", "accent_glow")
        ...
        self._add_hover_effect(self.desktop_btn, "glass", "#2a6496")
    ...
    # ✅ FIX #2: Режим "Все окна" — СОХРАНЯЕМ СОСТОЯНИЕ перед изменением
    def _toggle_all_windows_mode(self):
        ...
        self._pre_all_windows_state = {
            "cur_space": self.cur_space,
            "show_all_windows_mode": self.show_all_windows_mode,
            "spaces_snapshot": [
                {"windows": [w.copy() for w in sp["windows"]]} 
                for sp in self.spaces
            ]
        }
        self._all_windows_was_active = True
    ...
    # ✅ FIX #1: Ховер с безопасными цветами
    def _add_hover_effect(self, widget, base_key: str, hover_key: str):
        def on_enter(e):
            try:
                color = WATER_COLORS.get(hover_key, hover_key)
        ...
        def on_leave(e):
            try:
                color = WATER_COLORS.get(base_key, base_key)
                if color != "transparent":
                    widget.configure(fg_color=color)
            except:
                pass
    ...
    def _should_minimize(self, hwnd: int) -> bool:
        ...
        try:
            ...
            skip = ["shell_traywnd", "progman", "windows.ui.core", 
                    "applicationframehost", "searchui", "cortana",
                    "smart desktop"]
            if any(s in cls or s in title for s in skip):
                return False
        except:
            return False
        return True
    ...
    # ✅ FIX #1: ПЕРЕ СОЗДАНИЕ кнопок для правильного highlighting
    def _switch_space_view(self, idx: int):
        """Только меняет отображаемое пространство в панели, не трогая окна на экране"""
        if idx >= len(self.spaces) or self.cur_space == idx:
            return
        if self.show_all_windows_mode:
            self.show_all_windows_mode = False
            self._all_windows_was_active = False
        ...
        self.cur_space = idx
        # ✅ ПЕРЕ СОЗДАЁМ кнопки — это гарантирует правильные цвета в customtkinter
        if hasattr(self, 'space_buttons_container'):
            self._update_space_buttons()
        self._render_windows_list()
        ...
        self._on_activity()
        logger.info(f"Switched VIEW to space {idx + 1}")

    def _apply_space_layout(self, idx: int):
        """Применяет расстановку: сворачивает другие, восстанавливает и РАССТАВЛЯЕТ целевое пространство"""
        if idx >= len(self.spaces):
            return
        
        # ✅ Если был активен режим "все окна" — выходим из него
        if self._all_windows_was_active and self._pre_all_windows_state:
            self.show_all_windows_mode = False
            self._all_windows_was_active = False
            if hasattr(self, 'all_windows_btn'):
                self.all_windows_btn.configure(
        ...
        # Сначала переключаем ВИД
        self._switch_space_view(idx)
        
        # Затем манипулируем окнами на экране
        target_space = self.spaces[idx]
        target_hwnds = {win["hwnd"] for win in target_space["windows"]}
        
        if self.settings["behavior"]["minimize_other_spaces"]:
            for hwnd, title in self.available:
                if hwnd not in target_hwnds and self._should_minimize(hwnd):
                    try:
        ...
        for win in target_space["windows"]:
            try:
        ...
        # ✅ УМНАЯ РАССТАНОВКА: главное окно слева, остальные справа
        self._smart_arrange_windows(target_space["windows"])
        
        logger.info(f"Applied layout for space {idx + 1}: {len(target_space['windows'])} windows arranged")

    def _smart_arrange_windows(self, windows: List[Dict]):
        """✅ УМНАЯ расстановка: избранные слева поровну, остальные справа"""
        if not windows:
            return
        ...
        # ✅ Разделяем окна
        saved = [w for w in windows if w.get("saved_rect")]
        auto = [w for w in windows if not w.get("saved_rect")]
        
        # ✅ Если ВСЕ окна имеют saved_rect — просто применяем их
        if len(saved) == len(windows) and len(auto) == 0:
            for win in saved:
                x, y, w, h = win["saved_rect"]
                move_window(win["hwnd"], x, y, w, h)
            logger.info(f"Applied saved positions: {len(saved)} windows")
            return
        
        n = len(windows)
        
        # ✅ Разделяем на избранные и обычные
        main_wins = [w for w in windows if w.get("is_main")]
        other_wins = [w for w in windows if not w.get("is_main")]
        
        n_main = len(main_wins)
        n_other = len(other_wins)
        
        # ✅ Логика расстановки
        if n_main == 0:
            # Нет избранных — расставляем все как обычно
            if n == 1:
                move_window(windows[0]["hwnd"], gap, gap, screen_width - gap*2, screen_height - gap*2)
            elif n == 2:
                w = (screen_width - gap * 3) // 2
                move_window(windows[0]["hwnd"], gap, gap, w, screen_height - gap*2)
                move_window(windows[1]["hwnd"], gap + w + gap, gap, w, screen_height - gap*2)
            elif n == 3:
                w = (screen_width - gap * 3) // 2
                h_top = int(screen_height * 0.60)
                move_window(windows[0]["hwnd"], gap, gap, w, h_top - gap)
                move_window(windows[1]["hwnd"], gap + w + gap, gap, w, h_top - gap)
                move_window(windows[2]["hwnd"], gap, h_top, screen_width - gap*2, screen_height - h_top - gap)
            elif n == 4:
                w = (screen_width - gap * 3) // 2
                h = (screen_height - gap * 3) // 2
                coords = [
                    (gap, gap, w, h),
                    (gap + w + gap, gap, w, h),
                    (gap, gap + h + gap, w, h),
                ...
                # ✅ Оптимальная сетка для 5+ окон
                if n == 5:
                    # 3 сверху, 2 снизу
                    w = (screen_width - gap * 4) // 3
                    h_top = int(screen_height * 0.55)
                    h_bottom = screen_height - h_top - gap*2
                    # Верхний ряд (3 окна)
                    for i, win in enumerate(windows[:3]):
                        move_window(win["hwnd"], gap + i*(w+gap), gap, w, h_top - gap)
                    # Нижний ряд (2 окна по центру)
                    w2 = (screen_width - gap * 3) // 2
                    offset = (screen_width - (w2*2 + gap)) // 2
                    move_window(windows[3]["hwnd"], offset, h_top, w2, h_bottom)
                    move_window(windows[4]["hwnd"], offset + w2 + gap, h_top, w2, h_bottom)
                else:
                    # Для 6+ окон
                    cols = 3 if n >= 6 else 2
                    rows = math.ceil(n / cols)
                    cell_w = (screen_width - gap * (cols + 1)) // cols
                    cell_h = (screen_height - gap * (rows + 1)) // rows
                    for i, win in enumerate(windows):
                        r, c = divmod(i, cols)
                        move_window(win["hwnd"], gap + c*(cell_w+gap), gap + r*(cell_h+gap), cell_w, cell_h)
        
        elif n_main == 1:
            # Одно избранное — занимает 60% слева
            mw = int(screen_width * 0.60)
            move_window(main_wins[0]["hwnd"], gap, gap, mw - gap*2, screen_height - gap*2)
            
            # Остальные справа
            if n_other == 1:
                move_window(other_wins[0]["hwnd"], mw, gap, screen_width - mw - gap*2, screen_height - gap*2)
            elif n_other == 2:
                rw = screen_width - mw - gap * 2
                rh = (screen_height - gap * 3) // 2
                move_window(other_wins[0]["hwnd"], mw, gap, rw, rh)
                move_window(other_wins[1]["hwnd"], mw, gap + rh + gap, rw, rh)
            elif n_other >= 3:
                # ✅ Оптимальная сетка справа
                rw = screen_width - mw - gap * 2
                rh = screen_height - gap * 2
                
                if n_other == 5:
                    # 3 сверху, 2 снизу
                    cell_w = (rw - gap * 4) // 3
                    h_top = int(rh * 0.55)
                    h_bottom = rh - h_top - gap*2
                    # Верхний ряд (3 окна)
                    for i, win in enumerate(other_wins[:3]):
                        move_window(win["hwnd"], mw + gap + i*(cell_w+gap), gap, cell_w, h_top - gap)
                    # Нижний ряд (2 окна)
                    cell_w2 = (rw - gap * 3) // 2
                    move_window(other_wins[3]["hwnd"], mw + gap, gap + h_top, cell_w2, h_bottom)
                    move_window(other_wins[4]["hwnd"], mw + gap + cell_w2 + gap, gap + h_top, cell_w2, h_bottom)
                else:
                    # Для 3, 4, 6+ окон
                    cols = 2 if n_other <= 4 else 3
                    rows = math.ceil(n_other / cols)
                    cell_w = (rw - gap * (cols + 1)) // cols
                    cell_h = (rh - gap * (rows + 1)) // rows
                    for i, win in enumerate(other_wins):
                        r, c = divmod(i, cols)
                        move_window(win["hwnd"], mw + gap + c*(cell_w+gap), gap + r*(cell_h+gap), cell_w, cell_h)
        
        elif n_main >= 2:
            # ✅ НЕСКОЛЬКО избранных — делим левую часть ПОРОВНУ!
            left_width = int(screen_width * 0.50)  # Левая половина для избранных
            
            # ✅ Избранные окна слева — делим по вертикали
            main_h = (screen_height - gap * (n_main + 1)) // n_main
            
            for i, win in enumerate(main_wins):
                y_pos = gap + i * (main_h + gap)
                move_window(win["hwnd"], gap, y_pos, left_width - gap*2, main_h)
            
            # ✅ Остальные окна справа
            if n_other > 0:
                right_x = left_width + gap
                right_width = screen_width - left_width - gap*2
                
                if n_other == 1:
                    move_window(other_wins[0]["hwnd"], right_x, gap, right_width, screen_height - gap*2)
                elif n_other == 2:
                    rh = (screen_height - gap * 3) // 2
                    move_window(other_wins[0]["hwnd"], right_x, gap, right_width, rh)
                    move_window(other_wins[1]["hwnd"], right_x, gap + rh + gap, right_width, rh)
                elif n_other == 5:
                    # ✅ 3 сверху, 2 снизу — БЕЗ ДЫР!
                    cell_w = (right_width - gap * 4) // 3
                    h_top = int((screen_height - gap * 2) * 0.55)
                    h_bottom = screen_height - h_top - gap*3
                    # Верхний ряд (3 окна)
                    for i, win in enumerate(other_wins[:3]):
                        move_window(win["hwnd"], right_x + gap + i*(cell_w+gap), gap, cell_w, h_top - gap)
                    # Нижний ряд (2 окна по центру)
                    cell_w2 = (right_width - gap * 3) // 2
                    offset = (right_width - (cell_w2*2 + gap)) // 2
                    move_window(other_wins[3]["hwnd"], right_x + offset, gap + h_top, cell_w2, h_bottom)
                    move_window(other_wins[4]["hwnd"], right_x + offset + cell_w2 + gap, gap + h_top, cell_w2, h_bottom)
                else:
                    # Для 3, 4, 6+ окон
                    cols = 2 if n_other <= 4 else 3
                    rows = math.ceil(n_other / cols)
                    cell_w = (right_width - gap * (cols + 1)) // cols
                    cell_h = (screen_height - gap * (rows + 1)) // rows
                    for i, win in enumerate(other_wins):
                        r, c = divmod(i, cols)
                        move_window(win["hwnd"], right_x + gap + c*(cell_w+gap), gap + r*(cell_h+gap), cell_w, cell_h)
        
        logger.info(f"Smart arranged: {n} windows ({n_main} main, {n_other} other) - FULL coverage!")
    ...
    def _update_available_windows(self, windows_list):
        if not self.panel_expanded and not self.show_all_windows_mode:
    ...
    def _render_windows_list(self):
        ...
        if self.show_all_windows_mode:
            ...
            for hwnd, title in self.available:
                ...
                is_main = False
                if space_info:
                    is_main = space_info["data"].get("is_main", False)
        ...
        for item in windows_to_render:
            ...
            self._add_hover_effect(btn_check, "transparent", "glass")
            for widget in [btn_check, label, row]:
                try:
                    ...
                    pass
            show_star = False
            if in_space and not self.show_all_windows_mode:
                show_star = True
            elif self.show_all_windows_mode and space_idx is not None and is_main:
                show_star = True
            if show_star:
                ...
                self._add_hover_effect(btn_main, "transparent", "glass_border")
    ...
    def _toggle_main(self, hwnd):
        current_space = self.spaces[self.cur_space]
        for win in current_space["windows"]:
            if win["hwnd"] == hwnd:
    ...
    def _save_layout(self):
        ...
        for win in space["windows"]:
            ...
            try:
                ...
                work_left, work_top, _, _ = self._get_work_area()
                # ✅ Сохраняем относительно рабочей области
                win["saved_rect"] = (left - work_left, top - work_top, right - left, bottom - top)
                saved += 1
            except Exception as e:
    ...
    def _apply_layout(self):
        self._apply_space_layout(self.cur_space)
    ...
    # ✅ FIX #1: ПЕРЕ СОЗДАНИЕ кнопок — это решает проблему highlighting в customtkinter
    def _update_space_buttons(self):
        ...
        for i in range(min(self.max_spaces, 10)):
            # ✅ Вычисляем is_active ПЕРЕД созданием кнопки
            is_active = (i == self.cur_space and not self.show_all_windows_mode)
            btn = ctk.CTkButton(
                self.space_buttons_container, 
                text=f"{planet_icons[i]} {i+1}", 
                width=45, height=38,
                font=ctk.CTkFont(size=13, weight="bold"),
                # ✅ Цвета задаются ПРИ СОЗДАНИИ — customtkinter кэширует их
                fg_color=WATER_COLORS["accent"] if is_active else WATER_COLORS["glass"],
                hover_color=WATER_COLORS["accent_glow"] if is_active else WATER_COLORS["glass_border"],
                text_color="#000" if is_active else WATER_COLORS["text"],
                corner_radius=8, 
                command=lambda idx=i: self._switch_space_view(idx)
            )
            btn.pack(side="left", padx=4)
            self.space_btns.append(btn)
            # ✅ Ховер с ключами
            base = "accent" if is_active else "glass"
            hover = "accent_glow" if is_active else "glass_border"
            self._add_hover_effect(btn, base, hover)
...
if __name__ == "__main__":
    ...
    logger.info("=== Smart Desktop v1.4 Starting ===")
    app = SmartDesktop()
    try:
        app.mainloop()
